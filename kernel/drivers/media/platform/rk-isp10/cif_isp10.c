...
static int cif_isp10_set_pm_state(
	...
	enum cif_isp10_pm_state pm_state)
{
	dev_info(dev->dev, " cif_isp10_set_pm_state %s -> %s\n",
	...
	for (index = 0;; index++) {
		...
		if ((img_src_width >= target_width) &&
			...
			if (!matching_format_found ||
				...
				request_strm_fmt.frm_intrvl.denominator =
					strm_fmt_desc.min_intrvl.denominator;
				printk(KERN_INFO"VIDEO_TEST CIF_ISP10.c better match denom %d \n",request_strm_fmt.frm_intrvl.denominator);
		...
		// FIXME::GST set fomat(any@32768x32768) failed, force pass
			...
			request_strm_fmt.frm_intrvl.denominator =
				strm_fmt_desc.min_intrvl.denominator;
			printk(KERN_INFO"VIDEO_TEST CIF_ISP10.c else match denom %d \n",request_strm_fmt.frm_intrvl.denominator);
	...
	struct cif_isp10_device *dev)
{
	...
	u32 i;
	enum cif_isp10_pix_fmt in_pix_fmt;
	printk(KERN_INFO"CONFIGURE MIPI \n");
	...
	printk(KERN_INFO"MIPI CSI-2 DATA FORMAT CONFIGURED IS %x \n",data_type);
	cif_iowrite32(
	...
	u32 stream_ids)
{
	...
	if ((stream_ids & CIF_ISP10_STREAM_MP) ||
		...
		printk(KERN_INFO" CIF SEt PM STATE \n");
		...
		if (!CIF_ISP10_INP_IS_DMA(dev->config.input_sel)) {
			/* configure sensor */
			printk(KERN_INFO" CONFIGURE IMAGE SENSOR \n");
...
)
{
	...
	printk(KERN_INFO"STREAM METADATA RESET \n");
	...
	bool start_mp)
{
	...
	if (!((start_mp &&
		...
		(start_sp &&
		(dev->sp_stream.state != CIF_ISP10_STATE_STREAMING)))) {
		printk(KERN_INFO"SP and MP states are not streaming \n");
		return 0;
	}
	if (CIF_ISP10_INP_IS_DMA(dev->config.input_sel) &&
		(dev->dma_stream.state < CIF_ISP10_STATE_READY)) {
	...
	/* Activate MI */
	printk(KERN_INFO"activating MI \n");
	...
	if ((dev->sp_stream.state != CIF_ISP10_STATE_STREAMING) &&
		...
		if (CIF_ISP10_INP_IS_MIPI(dev->config.input_sel)) {
			printk(KERN_INFO"activating MIPI \n");
		...
		if (CIF_ISP10_INP_NEED_ISP(dev->config.input_sel)) {
			printk(KERN_INFO"activating ISP \n");
			cif_iowrite32OR(CIF_ISP_CTRL_ISP_CFG_UPD |
	...
	dev_info(dev->dev,
		" cif_isp10_start SP state = %s, MP state = %s, DMA state = %s, img_src state = %s\n"
		"  MI_CTRL 0x%08x\n"
		"  ISP_CTRL 0x%08x\n"
		"  MIPI_CTRL 0x%08x\n",
		cif_isp10_state_string(dev->sp_stream.state),
	...
	u32 stream_ids)
{
	...
	printk(KERN_INFO"CIF ISP10 STREAM On \n");
	printk(KERN_INFO
	...
	dev_info(dev->dev,
		"After SENSOR START SP state = %s, MP state = %s, DMA state = %s\n",
		cif_isp10_state_string(dev->sp_stream.state),
		cif_isp10_state_string(dev->mp_stream.state),
		cif_isp10_state_string(dev->dma_stream.state));
