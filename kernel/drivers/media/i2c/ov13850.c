...
/* added here */
	...
	if (!IS_ERR(ov13850->pwr_snd_gpio))
			gpiod_set_value_cansleep(ov13850->pwr_snd_gpio, 1);
...
/*	if (!IS_ERR(ov13850->pwdn_gpio))
	...
	if (!IS_ERR(ov13850->pwr_snd_gpio))
			gpiod_set_value_cansleep(ov13850->pwr_snd_gpio, 0);
			 ...
			 const struct i2c_device_id *id)
{
	...
	ov13850->pwr_gpio = devm_gpiod_get (dev,"pwr", GPIOD_OUT_HIGH);
	if (IS_ERR(ov13850->pwr_gpio))
			dev_warn(dev, "Failed to get pwr-gpios\n");

	ov13850->pwr_snd_gpio = devm_gpiod_get (dev, "pwr_snd", GPIOD_OUT_HIGH);
	if (IS_ERR(ov13850->pwr_snd_gpio))
			dev_warn(dev, "Failed to get pwr-snd-gpios\n");
