	...
	int len)
{
	...
	printk(KERN_INFO"REGISTER LIST FILLED \n");
	kfree(msg);
	kfree(data);
	return 0;
}
	...
	int on)
{
	...
	if (on) {
		...
		if (v4l2_subdev_call(sd,
			...
			printk(KERN_INFO"PLTFRM_CIFCAM_G_ITF_CFG success mclk is %d \n",itf_cfg.mclk_hz);
		} else {
			pltfrm_camera_module_pr_err(sd,
	...
	void *arg)
{
	...
	if (cmd == RK_VIDIOC_CAMERA_MODULEINFO) {
		...
		printk(KERN_INFO"platform ioctl RK_VIDIOC_CAMERA_MODULEINFO \n");
		/*
		...
		unsigned int i;
		printk(KERN_INFO"platform ioctl PLTFRM_CIFCAM_G_DEFRECT \n");
		for (i = 0; i < 4; i++) {
		...
		printk(KERN_INFO"platform ioctl PLTFRM_CIFCAM_G_ITF_CFG \n");
		...
		printk(KERN_INFO"platform ioctl PLTFRM_CIFCAM_ATTACH \n");
