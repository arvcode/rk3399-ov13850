	...
	void *arg)
{
	...
	if (cmd == RK_VIDIOC_SENSOR_MODE_DATA) {
		...
		printk(KERN_INFO"OV CAMERA IOCTL RK_VIDIOC_SENSOR_MODE_DATA \n");
		...
		printk(KERN_INFO"OV CAMERA IOCTL PLTFRM_CIFCAM_G_ITF_CFG \n");
		...
		*itf_cfg = config->itf_cfg;
		...
		printk(KERN_INFO"OV CAMERA IOCTL PLTFRM_CIFCAM_ATTACH \n");
		...
		struct v4l2_subdev *af_ctrl;
		printk(KERN_INFO"OV CAMERA IOCTL PLTFRM_CIFCAM_VCM_INFO \n");
		af_ctrl = pltfrm_camera_module_get_af_ctrl(sd);
		if (!IS_ERR_OR_NULL(af_ctrl)) {
			ret = v4l2_subdev_call(af_ctrl,
	...
	struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	printk(KERN_INFO"VIDEO_TEST [ov_camera_module_enum_frameintervals] DENOMINATOR %d \n",fie->interval.denominator);
	return 0;
}
