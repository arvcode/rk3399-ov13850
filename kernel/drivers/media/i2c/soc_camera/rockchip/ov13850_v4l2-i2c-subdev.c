...
/* Rewrite again for 4K */
	...
	{OV_CAMERA_MODULE_REG_TYPE_DATA,	0x5401,	0x61},
	{OV_CAMERA_MODULE_REG_TYPE_DATA,	0x5405,	0x40},
	{OV_CAMERA_MODULE_REG_TYPE_DATA,	0x380c,	0x25},
...
static int ov13850_start_streaming(struct ov_camera_module *cam_mod)
{
	...
	printk(KERN_INFO
		"start streaming active config=%s\n", cam_mod->active_config->name);
	...
	printk(KERN_INFO
			"WAIT BEFORE START STREAMING=%s\n", cam_mod->active_config->name);

	if (IS_ERR_VALUE(ov_camera_module_write_reg(cam_mod, 0x0100, 1)))
		goto err;
	printk(KERN_INFO
				"Written to I2c START STREAMING=%s\n", cam_mod->active_config->name);
	msleep(25);
/*	msleep(2000);
